...
const monthlyNewCustomersChart = document.getElementById(
  ...
  const chartOptions = {
    ...
    scales: {
      y: {
        ...
        suggestedMin: 0, // Đảm bảo giá trị tối thiểu của trục Y là 0
        ticks: {
          min: 0, // Đặt giá trị nhỏ nhất cho trục Y là 0
        },
      },
    },
  };
...
customerForm.addEventListener("submit", async function (event) {
  ...
  try {
    ...
    const response = await fetch(
      "http://localhost:8080/admin/customer/create-customer/",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    const addCustomerModal = bootstrap.Modal.getInstance(
      document.getElementById("addCustomerModal")
    );
