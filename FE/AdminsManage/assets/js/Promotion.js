...
fetch("http://localhost:8080/admin/promotion", {
  ...
  .then((data) => {
    ...
    if (data.length === 0) {
      promotionListElement.innerHTML =
        '<tr><td colspan="3">Không có khuyến mãi nào</td></tr>';
    } else {
      data.forEach((promotion) => {
        ...
        row.innerHTML = `
          ...
          <td>${promotion.promotionDetails}</td>
          <td>
  ...
  .catch((error) => {
    ...
    const promotionListElement = document.getElementById("promotion-list");
    promotionListElement.innerHTML =
      '<tr><td colspan="3">Lỗi khi tải khuyến mãi</td></tr>';
  });
  ...
  fetch(`http://localhost:8080/admin/promotion/?promotionID=${promotionID}`)
    ...
    .then((data) => {
      if (data) {
        ...
        document.getElementById("promotion-details").value =
...
function loadProductsByPromotion(promotionID) {
  ...
  fetch(`http://localhost:8080/admin/promotion-product/${promotionID}`, {
    ...
    .then((data) => {
      ...
      if (data.length === 0) {
        productListElement.innerHTML =
          '<tr><td colspan="5">Không có sản phẩm nào áp dụng khuyến mãi</td></tr>';
      } else {
        data.forEach((product) => {
          ...
          row.innerHTML = `
            ...
            <td>
              <input type="checkbox" ${
                product.hasPromotion === 1 ? "checked" : ""
              } 
    ...
    .catch((error) => {
      console.error("Lỗi khi gọi API:", error);
      productListElement.innerHTML =
        '<tr><td colspan="5">Danh sách trống</td></tr>';
    });
}
...
function togglePromotion(productID, isChecked) {
  if (selectedPromotionID) {
    ...
    fetch(url, {
      ...
      body: isChecked
        ? JSON.stringify({ productID, selectedPromotionID, isApplied: true })
        : null,
    })
      .then((response) => response.json())
      .then((data) => {
