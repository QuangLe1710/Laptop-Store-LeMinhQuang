...
const monthlyNewProductsChart = document.getElementById(
  "monthly-new-products-chart"
);
...
function displayProductList(products) {
  const productHTML = products
    .map((product, index) => {
      ...
      <tr>
        <td>${index + 1}</td>
        <td style="text-align:start;">${product.productName}</td>
        <td style="width: 15%;"><img src="${product.imageUrl}" alt="${
        product.productName
      }" style="width: 100px; height: auto;" /></td>
        <td style="width: 10%;">${product.productBrand}</td>
