...
deleteSelectedBtn.addEventListener("click", async () => {
  ...
  const supplierIDsToDelete = Array.from(selectedCheckboxes).map(
    (checkbox) => checkbox.closest("tr").cells[0].innerText // Lấy ID từ cột đầu tiên
  );
  ...
  const confirmDelete = confirm(
    `Bạn có chắc chắn muốn xóa ${supplierIDsToDelete.length} nhà cung cấp?`
  );
  if (confirmDelete) {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/admin/supplier/${ids}`,
        {
          method: "DELETE",
          headers: {
