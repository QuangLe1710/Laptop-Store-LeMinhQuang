...
function loadProductDetails(productID) {
  ...
  fetch(productApiUrl, {
    ...
    .then((data) => {
      ...
      console.log(
        "id của productInWarehouse là :",
        currentproductInWareHouseId
      );
...
function saveProductChanges() {
  ...
  const updatedProduct = {
    ...
    releaseDate:
      document.getElementById("editProductReleaseDate").value +
      "T00:00:00.000+00:00", // Đảm bảo định dạng
    warrantyPeriod: parseInt(
      document.getElementById("editProductWarranty").value
    ),
    ...
    minStockLevel: parseInt(
      document.getElementById("editProductMinStockLevel").value
    ),
    maxStockLevel: parseInt(
      document.getElementById("editProductMaxStockLevel").value
    ),
  ...
  // Gửi yêu cầu PUT
  fetch(
    `http://localhost:8080/admin/warehouse/update-product/${currentProductID}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
function loadWarehouseData(warehouseID) {
  ...
  fetch(warehouseApiUrl, {
    ...
    .then((data) => {
      ...
      document.getElementById("warehouseName").innerText =
        data.warehouseInfo.warehouseName;
      document.getElementById(
        "warehouseID"
      ).innerText = `${data.warehouseInfo.warehouseID}`;
      document.getElementById(
        "warehouseAddress"
      ).innerText = `${data.warehouseInfo.address}`;
      document.getElementById(
        "warehouseType"
      ).innerText = `${data.warehouseInfo.warehouseType}`;
      document.getElementById(
        "warehouseStatus"
      ).innerText = `${data.warehouseInfo.status}`;

      // Cập nhật số lượng sản phẩm trong kho
      document.getElementById("productQuantity").innerText = data.totalQuantity;
      document.getElementById("countProductsMaxStockLevel").innerText =
        data.countProductsMaxStockLevel;
      document.getElementById("countProductsMinStockLevel").innerText =
        data.countProductsMinStockLevel;
      ...
      data.listProductsInWarehouse.forEach((product) => {
        productListHtml += `
                        <li>
                            <img src="${
                              product.productImage || "default-image.jpg"
                            }" alt="${
          product.productName
        }" width="50" height="50">
                            <span>${product.productName}</span> - 
                            <span>Số lượng: ${product.quantity}</span>
                            <button class="btn btn-warning btn-sm" id="edit-button-${
                              product.productId
                            }" data-product-id="${
          product.productId
        }">Sửa</button>
    ...
    .then((data) => {
      ...
      if (data) {
        ...
        document.getElementById("editWarehouseName").value =
          data.warehouseName || "";
        document.getElementById("editWarehouseAddress").value =
          data.address || "";
        document.getElementById("editWarehouseType").value =
          data.warehouseType || "";
        document.getElementById("editWarehouseStatus").value =
          data.status || "";
