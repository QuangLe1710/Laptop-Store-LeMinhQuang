...
async function fetchAndOpenEditForm(row) {
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:8080/admin/supplier/update/${supplierID}`,
      {
        method: "GET",
        headers: {
    ...
    document.getElementById("edit-representative").value =
      supplier.representative; // Representative
    document.getElementById("edit-partnership-date").value =
      supplier.partnershipStartDate.split("T")[0]; // Date
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const updatedData = {
      ...
      partnershipStartDate: document.getElementById("edit-partnership-date")
        .value,
